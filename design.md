...
- [ ] CI green — `.github/workflows/ci.yml` written (backend `mvn verify` on Java 25 + NLP pytest, two parallel jobs). Every command in it has been run and passes locally; the workflow itself has **not** run on GitHub's infrastructure, since this isn't a Git repository yet — nothing to push to. Unchecking until it's actually gone green in a real Actions run.
